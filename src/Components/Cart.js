...
export default function Cart({ listItems, totalOrder, MenuName }) {
    ...
    return (
        <div className="p-1 rounded border border-warning "  >
            <div className='text-center bg-warning fs-5 fw-bold my-2 px-auto  rounded position-sticky'>Your Order</div>
            <ul className="list-unstyled ">
                {
                    listItems.map((item, index) =>
                        <li className='col-12 border d-flex mx-auto px-auto rounded' key={index}>
                            <div className='col-5 mx-auto px-auto py-1 fs-5  '>{item.name}</div>
                ...
                <Link to={`/Menu/${MenuName}/Payment`} className='col-12 border border-3 d-flex my-1  nav-link bg-warning rounded reflective-btn'><div className='col-5 mx-auto px-auto py-1 fs-5  fw-bold text-black '>Go For Payment</div></Link>
                </>:''}
